...
// validateTunName, validTunName, defaultTunName and tunNameHelp are per-OS —
// see tunname_darwin.go and tunname_windows.go. A utunN unit number is a real
// kernel constraint on macOS and nothing else, so the rule (and the flag
// default it validates) cannot be shared code without either lying to Windows
// users about what a tunnel device there is called or silently accepting a
// name macOS would never hand out.
