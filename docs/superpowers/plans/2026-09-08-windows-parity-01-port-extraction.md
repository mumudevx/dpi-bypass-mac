...
//
// There is no family field: the family is derived from Local (an address with a
// colon in it is v6), because a flag beside the address could disagree with the
// address the kernel is actually given.
type IfaceConfig struct {
	Local string
	Peer  string // v4 point-to-point peer; empty for v6, which takes a prefix length
	MTU   int
}
...
type IfaceController interface {
	...
	// calls would change the argv the kernel sees, which ops_test.go asserts
	// byte-for-byte.
	...
	// descriptor and vanishes when they close it.
